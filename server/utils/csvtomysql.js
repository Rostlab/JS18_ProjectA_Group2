	...
	import: function(csvfn, options){
		...
		new Promise((resolve, reject) => {
		    ...
		    return new Promise((resolve, reject) => {
		        context.db = mysql.createConnection(options.mysql);
		...
		.then(context => {
		    ...
		    return new Promise((resolve, reject) => {
		        context.db.query(`DROP TABLE IF EXISTS ${options.table}`,
		        [ ],
		        err => {
		            if (err) reject(err);
		    ...
		    return new Promise((resolve, reject) => {
		        ...
		        context.db.query(`CREATE TABLE IF NOT EXISTS ${options.table} ( ${fields} )`,
		        [ ],
		        err => {
		            if (err) reject(err);
		            else resolve(context);
		    ...
		    return new Promise((resolve, reject) => {
		        ...
		        fs.createReadStream(csvfn).pipe(parse({
		            ...
		            async.eachSeries(data, (datum, next) => {
		                ...
		                if (d.length > 0) {
		                    context.db.query(`INSERT INTO ${options.table} ( ${context.fieldnms} ) VALUES ( ${context.qs} )`, d,
		                    err => {
